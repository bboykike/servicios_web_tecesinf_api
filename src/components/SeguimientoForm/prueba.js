...
import  VerMas from '../DetallesForm'
import  TableInforme from '../InformeForm'
import TableHistorial from '../HistorialForms'
import ImagenForm from '../UpImageForm'
...
const TablePrueba =()=>{
       ...
       const [cookiee, setCoo] = React.useState(['ID'])
        ...
        useEffect(() => {  
            const GetData = async () =>{
                const response =  await Api.get(`ServiciosLista/${ID}`)
    ...
    return(
      ...
      <Paper>
        ...
        <Table aria-label="collapsible table">
          ...
          <TableBody>
            {listado.map((row) => (
              <Row key={row.IDservicio} row={row} /> 
    ...
    function Row(props) {
      ...
      return (
        <>
          ...
          >
                    <DialogContent>  
                    <TableHistorial servicios={servicios} setEvidenciaOpen={ setHistorialOpen } />
                    </DialogContent>
          </Dialog>
        </> 
      );
